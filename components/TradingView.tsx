...
const TradingView: React.FC = () => {
  ...
  // Balances
  const usdtBalance = user.assets.find(a => a.symbol === 'USDT')?.amount || 0;
  const coinBalance = user.assets.find(a => a.symbol === selectedSymbol)?.amount || 0;
  ...
  useEffect(() => {
    const fetchChartHistory = async () => {
      try {
        // If simulated DMC, fetch BTC and scale it
        ...
        const formattedChartData: ChartPoint[] = data.map((d: any) => ({
          ...
          price: selectedSymbol === 'DMC' ? parseFloat(d[4]) * 0.010 : parseFloat(d[4])
        }));
    ...
    fetchChartHistory();
    // Refresh chart history every minute
    const interval = setInterval(fetchChartHistory, 60000);
    return () => clearInterval(interval);
  }, [selectedSymbol]); // Re-fetch when symbol changes

  // Update latest chart point with live socket price to make it feel real-time
  useEffect(() => {
    if (currentPrice > 0 && chartData.length > 0) {
      setChartData(prev => {
        ...
        // Create a copy of the last point to avoid "assign to read only property" error
  ...
  useEffect(() => {
      ...
      try {
        ...
        const mapOrders = (orders: string[][], type: 'buy' | 'sell'): Order[] => {
            return orders.map(o => {
                ...
                // Scale for DMC
                const finalPrice = selectedSymbol === 'DMC' ? rawPrice * 0.010 : rawPrice;
                // If price is lower, volume usually higher, simplify by keeping amount same for visualization
        ...
        setAsks(mapOrders(data.asks, 'sell').reverse()); // Highest sells on top
    ...
    fetchOrderBook();
    const interval = setInterval(fetchOrderBook, 3000); // Refresh every 3 seconds
    return () => clearInterval(interval);
  }, [selectedSymbol]);

  // Reset inputs when tab changes or symbol changes
  useEffect(() => {
    setAmount('');
    setTotal('');
    setSliderValue(0);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-1 min-h-[calc(100vh-64px)] bg-[#0b0e11] p-1">
      ...
      {/* Right Column: Trade Form (Desktop) */}
      <div className="bg-binance-black p-4 col-span-1 border border-binance-gray/20 rounded-sm">
             ...
             <div className="flex justify-between text-xs text-binance-subtext mb-1">
                 ...
                 >
                     {activeTab === 'buy' 
                        ...
                        : `${coinBalance.toFixed(6)} ${selectedSymbol}`}
                 </button>
             </div>
